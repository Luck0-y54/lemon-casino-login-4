...
return order;}
function buildGrid(){gridEl.innerHTML="";state.tiles=[];for(var i=0;i<TILE_COUNT;i+=1){var tile=document.createElement("button");tile.type="button";tile.className="tile-cf93afcc";tile.disabled=true;tile.setAttribute("aria-label","Tile "+(i+1));tile.innerHTML='<span class="tile-inner-cf93afcc">'+'<span class="tile-face-cf93afcc tile-front-cf93afcc"></span>'+'<span class="tile-face-cf93afcc tile-back-cf93afcc"></span>'+"</span>";tile.addEventListener("click",makeTileHandler(i));gridEl.appendChild(tile);state.tiles.push(tile);}}
function makeTileHandler(index){return function(){onTilePick(index);};}
function revealTile(index,isMine,dim){var tile=state.tiles[index];if(!tile||tile.classList.contains("revealed-cf93afcc")){return;}
var back=tile.querySelector(".tile-back-cf93afcc");if(back){back.innerHTML=isMine?BOMB_SVG:GEM_SVG;}
tile.classList.add("revealed-cf93afcc");if(isMine){tile.classList.add("boom-cf93afcc");}
if(dim){tile.classList.add("dimmed-cf93afcc");}
tile.disabled=true;}
function revealRemaining(dimAll){for(var i=0;i<TILE_COUNT;i+=1){if(!state.tiles[i].classList.contains("revealed-cf93afcc")){revealTile(i,state.mineMap[i]===true,dimAll);}}}
function setTilesEnabled(enabled){for(var i=0;i<TILE_COUNT;i+=1){var tile=state.tiles[i];tile.disabled=!enabled||tile.classList.contains("revealed-cf93afcc");}}
...
var tile=state.tiles[index];if(!tile||tile.classList.contains("revealed-cf93afcc")){return;}
...
function bust(){var lost=state.bet;revealRemaining(true);setStatus("Boom! The mine took your "+formatMoney(lost)+" bet.","lose");if(panelEl){panelEl.classList.remove("jolt-cf93afcc");void panelEl.offsetWidth;panelEl.classList.add("jolt-cf93afcc");}
endRound();}
